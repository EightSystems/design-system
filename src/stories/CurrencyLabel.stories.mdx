...
O `CurrencyLabel` usa a API de internacionalização `Intl.NumberFormat`, e possui duas propriedades obrigatórias: `currency` e `value`:
...
Adicionalmente, é possível especificar o tamanho a a unidade de medida para alterar o tamanho de exibição de `CurrencyLabel` com `size` e `sizeUnit`.
